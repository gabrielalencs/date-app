...
### D-110 — Foto de memória não emite evento, e a que vira capa continua memória
**13/09/2026.** Subir foto depois do date não entra no `activity_events`, pelo mesmo motivo do checklist no B8 (D-097): o feed é a história do plano, não o log de edição. E `setPlanCover` deixou de reescrever `media.purpose` quando a foto é `memory` — a autoridade sobre qual é a capa é `plans.cover_media_id`, e o `purpose` só registra por onde a foto entrou. Sem esse recorte, a foto sairia da grade de "Como foi?" no exato momento em que vira capa, que é justamente o momento em que o card de `/memorias` deixa de mostrar a foto do site do restaurante.

### D-109 — Concluir emite; a primeira avaliação de cada pessoa emite; editar a nota não
**13/09/2026.** Os dois verbos já existiam no enum desde o B2 — `plan_completed` e `memory_added` —, então o bloco não precisou de migration nem de investigar `ALTER TYPE ... ADD VALUE` dentro de transação. `memory_added` sai quando a linha de avaliação **nasce**, e mais nenhuma vez: o feed do B10 não conta quantas vezes alguém mudou de ideia sobre um jantar de três meses atrás. Diverge de propósito do `vote_cast` do B6, que emite a cada mudança — lá a mudança de voto é a negociação acontecendo e faz parte da história; aqui não é. O B6 fica como está. Provado contando as linhas antes e depois: três edições seguidas não acrescentam nenhuma, e retirar e reavaliar acrescenta uma, porque a linha nasceu outra vez.

### D-108 — O número de consultas da timeline é constante em relação ao número de planos
**13/09/2026.** Quatro consultas por página, com uma memória ou com sessenta: os planos da página, os membros do workspace, as avaliações em bloco e a contagem de fotos em bloco. Tudo por `inArray` sobre os ids da página, nunca num laço. A exigência é medida e não opinião, porque com os oito planos do seed uma timeline que consulta por linha responde igual a uma que consulta em bloco — nada ficaria vermelho na máquina de ninguém. A prova usa fixture de sessenta planos realizados, criada e removida pelo próprio teste, e instrumenta `database.$client.query`. Defeito plantado: trocando a contagem de fotos por uma consulta por linha, a página cheia passa a 27 consultas e a última página a 21, e o teste fica vermelho.

### D-107 — Paginação da timeline por número de página na URL
**13/09/2026.** `/memorias?pagina=2`, navegada por link como o mês da agenda (D-078): cada página é uma URL de verdade, a leitura funciona sem JavaScript e o histórico se comporta. Cursor seria mais correto num feed vivo, mas esta lista é de passado e praticamente imóvel — um plano só entra nela quando alguém marca um date como realizado, o que acontece uma vez por date. Cursor composto de instante mais id numa URL é feio e não compra nada aqui. Página inválida cai na primeira sem tela de erro, e o parser tem teto de sete dígitos para `?pagina=99999999999` não virar um `OFFSET` que estoura o `integer` do Postgres.

### D-106 — A timeline consome a aritmética de dia civil do B7; nenhuma função de data nova
**13/09/2026.** O agrupamento por mês usa `monthOf` e `formatMonthTitle`, que já existiam, e a zona do ESLint (D-073) garante que nada escape. Um date às 23:00 de 30 de setembro é `2026-10-01T02:00Z`: agrupado por UTC ele muda de mês, e a timeline erraria o mês de metade dos dates deste produto, que são noturnos. A suíte do agrupamento entrou no `test:tz` e roda nos três fusos. Defeito plantado: trocando `monthOf` por `getUTCFullYear`/`getUTCMonth`, duas asserções ficam vermelhas nos três fusos — inclusive no local, porque `getUTC*` é sempre UTC — e o lint barra as duas propriedades, duas redes independentes.

### D-105 — Sem média com um avaliador só
**13/09/2026.** Com uma pessoa avaliando não existe média: existe a nota daquela pessoa. Mostrar "4,0" quando metade do casal não respondeu é uma mentira pequena que o bloco de estatísticas depois amplifica. A média é calculada em **décimos inteiros** e formatada a partir dos dígitos, sem passar por float, pelo mesmo motivo do `lib/money.ts` — com duas pessoas o resultado é sempre exato (x,0 ou x,5).

### D-104 — Avaliação por pessoa, as duas sempre visíveis, e retirar leva o "repetiria" junto
**13/09/2026.** As regras vêm inteiras do B6 (D-062): as duas avaliações são sempre visíveis, ausência é estado distinto de nota baixa e aparece como "Alex ainda não avaliou", e reenviar a mesma nota a retira. O que o B9 precisou decidir é o que acontece com o "repetiria?" nessa retirada: `memory_ratings.rating` é `NOT NULL` desde o B2, então não existe avaliação sem nota, e retirar apaga a linha inteira. Uma resposta de "repetiria" sozinha seria uma avaliação pela metade que a tela não saberia mostrar. Pelo mesmo motivo, o controle de "repetiria?" só aparece depois da nota, e a camada de dados recusa de novo — o frontend nunca é fonte de autoridade.

### D-103 — `gallery` é antes, `memory` é depois
**13/09/2026.** Mesma tabela, mesmo upload assinado, mesma rota autenticada de leitura, mesmo reprocessamento no cliente que descarta EXIF. Nada do B5 foi reescrito: `memory` entrou em `UPLOADABLE_PURPOSES`, que era a única coisa que faltava. A grade de memória não reordena — o que se fotografou depois do date é registro, e a ordem dele é a ordem em que aconteceu —, então `reorderPlanMedia` passou a operar só sobre as fotos que não são de memória, sem o que a lista fechada que a grade do plano envia nunca conferiria com o total do plano.

### D-102 — `completed` é terminal na transição e não na escrita
**13/09/2026.** Uma regra do tipo "status terminal é somente leitura" mataria metade deste bloco: avaliação e fotos de memória **só existem** depois de `completed`, e os gastos continuam editáveis porque é depois que se sabe quanto custou. A exceção declarada é o checklist, que vira leitura: "o que levar" perde a função no momento em que as duas pessoas já foram, e marcar um item depois não afirma nada sobre o mundo. Cancelado e arquivado continuam leitura em tudo, como no B8 (D-098). Votar numa data também some, pelo mesmo motivo: a negociação terminou.

### D-101 — Marcar como realizado é irreversível e passa por confirmação em modal
**13/09/2026.** Segundo uso sancionado do modal neste produto, ao lado da remoção de foto (D-080). `completed` é terminal de propósito: desfazer um date realizado deixaria a avaliação e as fotos penduradas num plano que voltou a ser ideia. O botão sai da fila de transições — as outras continuam sendo um clique só, porque se desfazem sozinhas — e o texto do modal diz que não tem volta, em português de gente e sem drama.

### D-100 — `planned`/`reserved` → `completed` exige data confirmada em dia civil não futuro
**13/09/2026.** `completed` afirma que o date aconteceu, e a regra do B8 (D-091) continua valendo: um status só é alcançável quando o fato que ele afirma existe. Marcar como realizado um date que é semana que vem não é caso de uso, é erro de digitação — e como a transição não tem volta, a recusa precisa vir antes e não depois. A comparação é de **dia civil**, pela aritmética do B7 (`isFutureCivilDay`), nunca por subtração de milissegundos: um date hoje às 23h ainda é hoje ao meio-dia, e marcar tem de funcionar. A pré-condição mora no módulo que o B8 criou e é consultada duas vezes, para oferecer e para escrever (D-092). `PlanFacts` ganhou um terceiro fato, `confirmedDateIsFuture`, e o módulo continua puro: quem lê o banco resolve a comparação e entrega o fato pronto.

### D-099 — Memória não é entidade nova: é o plano, depois
**13/09/2026.** Título, data, local, gastos e fotos não são copiados para lugar nenhum — a data é a opção confirmada do B6, o local está na `plans`, os gastos são do B8 e as fotos são a `media` do B5. O B2 já tinha acertado isso: `memories` tem apenas `plan_id`, `highlight` e `notes`, sem título, sem data e sem lugar. A linha de `memories` é criada sob demanda, na primeira avaliação ou no primeiro texto, e não na travessia para `completed`: criar ali deixaria de fora todo plano que já estivesse realizado antes deste bloco existir, e um "não encontrado" seria um erro que a pessoa não teria como corrigir. `highlight` e `notes` são do **casal**, uma por plano, e não de cada pessoa: a melhor parte de uma noite é uma coisa só, e duplicá-la por pessoa transformaria uma lembrança compartilhada em dois depoimentos paralelos.
