...
### D-159 — O backfill não reproduz o passado
**16/09/2026.** Ligar notificações não pode despejar o histórico do casal na tela bloqueada das duas pessoas de uma vez. O backfill faz uma coisa só: planos com data confirmada **futura** ganham os lembretes 7/5/3/1 que ainda estão no futuro. Sem isso, os dates já marcados antes do bloco nunca teriam lembrete e a feature só valeria para quem marcasse dali em diante. Idempotente pelo mesmo índice parcial que faz o debounce — medido em `development`: 16 lembretes na primeira execução, 0 na segunda.

### D-158 — `archivePlan` virou transação
**16/09/2026.** Era um UPDATE solto e continuaria correto assim. Mudou porque a intenção de notificar precisa nascer junto com o fato, e "junto" só existe dentro de uma transação — sem isso existiria a janela em que o plano foi arquivado e a notificação não foi prometida, ou o contrário. O UPDATE continua carregando o predicado de workspace e o `isNull(archived_at)`, então a idempotência não mudou.

### D-157 — O handler de push é provado em sandbox, não no navegador
**16/09/2026.** O Chromium headless reporta `Notification.permission === "denied"` de forma incondicional: medido com `permissions: ["notifications"]` no contexto do Playwright, com `grantPermissions` por origem e com flag de linha de comando, os três devolvendo `denied`. Um push entregue por `ServiceWorker.deliverPushMessage` nesse estado não chega a virar notificação, então um teste de navegador mediria a limitação do ambiente em vez do produto. `tests/service-worker-push.test.ts` carrega o `public/sw.js` **do disco** e dispara os eventos contra um `self` controlado — é o arquivo que vai para o ar, não uma reimplementação. A aceitação real em tela bloqueada é da lista do proprietário.

### D-156 — `NEXT_PUBLIC_` na chave pública do VAPID é exceção sancionada
**16/09/2026.** A regra do `CLAUDE.md` contra `NEXT_PUBLIC_` existe para segredo. A chave pública do VAPID não é segredo por construção: ela viaja dentro da própria `PushSubscription` que o navegador monta e entrega ao servidor, e sem ela no cliente não existe `applicationServerKey`. A privada é `server-only`, lida de `process.env` na primeira chamada do remetente — não no topo do módulo, para que um ambiente sem VAPID configurado não derrube o build.

### D-155 — `.well-known/workflow/` fica fora do matcher do proxy
**16/09/2026.** São as rotas internas que o runtime do Workflow chama para executar step e retomar run. Interceptá-las quebra a execução com `Cannot perform ArrayBuffer.prototype.slice on a detached ArrayBuffer`, porque o corpo é consumido antes de chegar ao handler; a documentação do SDK marca isto como fácil de errar justamente no Next 16, onde o `proxy.ts` substituiu o `middleware.ts`. Não é exceção de autorização: essas rotas não servem dado de produto e não aceitam `workspaceId` de quem chama. A única com superfície pública seria a de webhook do Workflow, e o DATE não cria nenhum — não usa hooks.

### D-154 — `preview_mode` nasce em `private`
**16/09/2026.** A tela bloqueada é pública para quem estiver perto da pessoa. O default não mostra título do date nem data: mostra "Tem novidade no DATE". Mostrar detalhe é opt-in consciente, com um interruptor no Perfil. A decisão é de produto e não de segurança — não protege contra ninguém com acesso ao aparelho —, mas é a diferença entre o nome do date de vocês aparecer numa mesa de reunião ou não.

### D-153 — `workflow` e `web-push` são as duas dependências acrescentadas
**16/09/2026.** `workflow@4.8.8` é o SDK do Vercel Workflows, GA desde abril de 2026, com `sleep(date)` aceitando data absoluta — que é exatamente o que `due_at` pede — e rodando sob `next dev` sem deploy. `web-push@3.6.7` é a implementação de referência do RFC 8291 mantida pela `web-push-libs` e a que o guia de PWA do próprio Next indica. Criptografia de Web Push escrita à mão seria erro de julgamento: ECDH, HKDF e AES-GCM com um envelope que precisa bater byte a byte com o que o navegador espera, e o sintoma de errar é silêncio. Firebase e OneSignal ficaram de fora: nenhum dos dois é necessário para duas pessoas, e os dois acrescentariam um terceiro que veria as notificações.

### D-152 — O único é parcial, e é ele que faz o debounce
**16/09/2026.** `(workspace_id, recipient_profile_id, dedupe_key)` é único **enquanto a intent está aberta** (`pending` ou `processing`). Quatro datas sugeridas dentro da janela de 20 minutos colidem na mesma linha, atualizam o `due_at` e viram uma notificação agregada; depois que aquela foi enviada ou suprimida, a próxima sugestão pode abrir outra com a mesma chave. Um único total seria um defeito silencioso: o primeiro "quero muito" de um plano impediria para sempre o segundo. A chave inclui o ator onde o debounce é por pessoa — sem isso, as duas pessoas sugerindo datas no mesmo plano se calariam mutuamente.

### D-151 — A fronteira de sistema, declarada e mínima
**16/09/2026.** `features/notifications/data/system.ts` é a única parte de `features/*/data/` que não recebe `AuthorizedContext`, porque quem a chama é o Workflow acordando de um sleep de sete dias ou o Cron de reparo — não existe pessoa. O que substitui a autorização é uma restrição mais **estreita**: a única entrada é um `intentId` já persistido, o `workspaceId` é lido da linha e nunca aceito de quem chamou, nenhuma função aceita filtro vindo de fora, e nada dali é exportado para Server Action, página ou componente. `docs/DATA_ACCESS.md` seção 3 continua valendo para todo o resto.

### D-150 — O Cron é reparo de outbox, nunca o agendador
**16/09/2026.** O agendamento é do Workflow. O Cron diário existe para um caso só: o `start()` falhou depois do commit e a intent ficou `pending` sem run. Ele não envia nada — reinicia o workflow, e quem decide é o claim. Protegido por `CRON_SECRET` comparado em tempo constante, e responde **404 quando a variável não existe**: falha fechada, porque uma rota de manutenção que funciona sem autenticação por esquecimento de variável é pior que uma rota ausente.

### D-149 — O input durável do workflow é só o `intentId`
**16/09/2026.** Nada de título, subscription, nome ou endpoint: o payload de um run fica gravado no log de eventos da plataforma, e o que não é gravado não vaza. Tudo o mais é lido do banco no instante do envio — que é, além de privacidade, a razão pela qual a notificação consegue ser verdadeira. O laço do workflow existe por causa do debounce: enquanto a pessoa continua agindo, a mutation empurra o `due_at` para frente na mesma linha, o run acorda, vê que ainda não é hora e volta a dormir, com teto de cinco voltas. A decisão de dormir é tomada **dentro de um step**, para o replay ler o veredito do log em vez de consultar o relógio de novo.

### D-148 — Notificar o estado estável, não o clique
**16/09/2026.** A tese do B11.5. Uma mutation não dispara mensagem: ela grava uma **intenção** na mesma transação do domínio, com os fatos mínimos de revalidação e um `due_at`. Quando a hora chega, o servidor relê o estado atual e só envia se a afirmação continuar verdadeira. Por isso `notification_intents` não tem coluna de título nem de corpo — guardar o texto no momento do clique seria congelar uma afirmação que ainda pode deixar de ser verdade. `suppressed` não é falha: é o produto funcionando, alguém mudou de ideia dentro do prazo e o telefone da outra pessoa não mentiu. O caso extremo é a data confirmada, que não notifica em minutos e sim às 09:00 do próximo dia civil: confirmar às 23h e desfazer às 8h não gera push nenhum.
