...
- **Segunda rodada de performance**, depois de o proprietário reportar que ainda estava lento com as primeiras correções já no ar: Motion trocado por CSS (−117 kB por rota, D-166), Zod tirado do bundle do cliente (build de 1306 para 938 kB, D-167) e pré-busca desligada na navegação (Home de 13 para 2 requisições RSC, `/ideias` para zero — D-168). Cada pré-busca era uma renderização completa no servidor.
- **Upload de foto quebrado em produção, segunda causa: `R2_ACCESS_KEY_ID` recebeu o Token value do Cloudflare** (D-170). Com o CORS aplicado, o PUT passou a chegar ao R2 e voltou `400 InvalidArgument`. A URL assinada mostrava `X-Amz-Credential=cfat_…` — o painel mostra três campos ao criar um token de R2 e só dois entram no ambiente; o `Token value` é da API REST e não assina requisição S3. Reproduzido contra o bucket de desenvolvimento: chave com prefixo `cfat_` → 400 `InvalidArgument`, 149 bytes, idêntico ao reportado; chave 32 hex inexistente → 401 `Unauthorized`. Correção é do proprietário, na Vercel. `resolveR2` passou a conferir o formato das duas credenciais e abortar com mensagem que nomeia o campo do painel. **O Access Key ID viaja em toda URL assinada: o token precisa ser girado.**
- **Upload de foto quebrado em produção: `date-media-prod` não tem CORS** (D-169). O `OPTIONS` do navegador é recusado com 403 para qualquer origem, então o PUT assinado nunca acontece. Correção é no painel do Cloudflare e é do proprietário; `pnpm r2:cors <url>` faz a verificação e imprime a política.
- **Performance do app instalado, investigada e tratada.** O proprietário reportou lentidão grave ao tocar depois de instalar a PWA. Três causas medidas e corrigidas: (1) a resolução de contexto fazia **duas** idas e voltas ao banco por requisição — um UPSERT e um SELECT — inclusive em cada `/api/media/[id]` de uma grade de fotos, e passou a ser uma consulta sem escrita no estado normal, de 28 ms para 12 ms (D-161); (2) navegar para rota dinâmica congelava a tela por até **898 ms** sem sinal nenhum, e agora uma barra responde ao toque em 57–70 ms via `useLinkStatus` (D-162); (3) a função rodava na região padrão da Vercel, nos Estados Unidos, contra um banco em `sa-east-1` — `vercel.json` passou a declarar `gru1` (D-164). As medidas estão no relatório; a confirmação em aparelho real é do B12.
...
| `pnpm r2:cors [url]` | faz o `OPTIONS` que o navegador faz antes do PUT; sem CORS, o upload quebra |
